...
class ChannelListViewController: V2TTableViewController, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate   {

    let channelCellReuseableIdentifier = "channelCell"
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier(channelCellReuseableIdentifier, forIndexPath: indexPath) as! ChannelTableViewCell
        cell.channel = channels.items[indexPath.row]
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        let channelVideoVC = self.channelVideoVC(self.channels.items[indexPath.row].id)
        ...
        titleLabel.text = self.channels.items[indexPath.row].title
        ...
        channelVideoVC.navigationItem.titleView = titleLabel
        
        self.navigationController?.pushViewController(channelVideoVC, animated: true)
    }
    ...
    */
    ...
    private func channelVideoVC(channelId: String) -> UIViewController {
        ...
        videoListVC.viewDidLoadHandler = {
            ...
            sharedYTAPI.getChannelItems(channelId, videos: Items(), completionHandler: { (videos) -> Void in
        ...
        videoListVC.scrollViewDidEndDraggingHandler = {
            if let _ = videoListVC.videos.nextPage {
                sharedYTAPI.getChannelItems(channelId, videos: videoListVC.videos, completionHandler: { (videos) -> Void in
                    videoListVC.videos = videos
                    videoListVC.tableView.reloadData()
                })
            }
